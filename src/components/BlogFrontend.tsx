...
import { 
  ...
  AlertCircle, Eye, Award, BookmarkCheck, ArrowUpRight, Zap, Sliders
} from 'lucide-react';
...
export const BlogFrontend: React.FC<BlogFrontendProps> = ({
  ...
  const { hue, speed, setSpeed, isPaused: isHuePaused, setIsPaused: setIsHuePaused, particleCount, setParticleCount, auroraOpacity, setAuroraOpacity } = useHue();
  ...
  const renderRefinedLandingPage = () => {
    return (
      <motion.div
        ...
        id="refined-landing-container"
      >
        {/* HERO SECTION */}
        <div className="relative overflow-hidden rounded-3xl border border-slate-850/60 bg-gradient-to-br from-slate-950 via-slate-950/90 to-slate-900/40 p-8 sm:p-12 lg:p-16 flex flex-col items-center text-center space-y-6">
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(251,191,36,0.04),transparent_60%)]" />
          
          <div className="relative z-10 flex flex-col items-center space-y-3">
            <span className="px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/20 text-[#fbbf24] text-[10px] uppercase font-black tracking-widest font-mono">
              ✦ Ambient Publishing Hub
            </span>
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black text-white tracking-tight leading-none font-sans">
              Welcome to <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-[#fbbf24] to-orange-400">AuraLife</span>
            </h1>
            <p className="text-xs sm:text-sm text-slate-400 font-mono tracking-widest uppercase mt-1">
              Deserving reads in an atmosphere of slow living & visual wellness
            </p>
          </div>

          <p className="relative z-10 max-w-2xl text-xs sm:text-sm text-slate-300 leading-relaxed font-sans">
            AuraLife is an editorial canvas curated to chronicle moments, slow living practices, sensory flavor pairings, and cognitive masteries. Complete with a real-time atmosphere tuner, a content workspace, and active reviewer directories.
          </p>

          <div className="relative z-10 flex flex-wrap justify-center gap-4 pt-2">
            <button
              onClick={onNavigateToAdmin}
              className="px-6 py-3 rounded-xl bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-500 hover:to-amber-600 text-white font-bold text-xs uppercase tracking-wider flex items-center gap-2 cursor-pointer shadow-[0_4px_20px_rgba(245,158,11,0.25)] border border-amber-500/25 transition-all hover:scale-[1.02]"
            >
            ...
            <a
              href="#vibe-sandbox"
              className="px-6 py-3 rounded-xl bg-slate-900 hover:bg-slate-850 text-slate-300 hover:text-white font-bold text-xs uppercase tracking-wider border border-slate-800 hover:border-slate-700 flex items-center gap-2 transition-all"
            >
              <Zap className="w-4 h-4 text-amber-400" />
              <span>Interactive Atmosphere Tuner</span>
            </a>
          </div>
        </div>

        {/* CURATED CHANNELS (BENTO GRID) */}
        <div className="space-y-6">
          <div className="text-center space-y-1.5">
            <h2 className="text-xl sm:text-2xl font-extrabold text-white tracking-tight">Our Curated Publishing Channels</h2>
            <p className="text-xs text-slate-500 font-mono">Four distinct focus territories built for quiet reading and learning</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {categories.map((cat, idx) => {
              const catHue = (hue + idx * 40) % 360;
              return (
                <div 
                  key={cat.id}
                  onClick={() => setRoute({ type: 'category', id: cat.id })}
                  className="group relative overflow-hidden rounded-2xl border border-slate-900 bg-slate-950 p-6 flex flex-col justify-between h-72 cursor-pointer hover:border-slate-800 hover:shadow-[0_8px_30px_rgba(0,0,0,0.8)] transition-all duration-300"
                >
                  {/* Category Image background with blending */}
                  <div className="absolute inset-0 opacity-15 group-hover:opacity-25 transition-opacity duration-500">
                    <img src={cat.coverImageUrl} className="w-full h-full object-cover" alt="" />
                    <div 
                      className="absolute inset-0 mix-blend-color"
                      style={{ backgroundColor: `hsl(${catHue}, 80%, 45%)` }}
                    />
                  </div>

                  <div className="relative z-10 space-y-3">
                    <span 
                      className="inline-block px-2 py-0.5 rounded text-[9px] font-black tracking-widest uppercase font-mono"
                      style={{ backgroundColor: `hsla(${catHue}, 80%, 15%, 0.8)`, color: `hsl(${catHue}, 90%, 65%)` }}
                    >
                      Channel 0{idx + 1}
                    </span>
                    <h3 className="text-base font-extrabold text-white group-hover:text-amber-400 transition-colors leading-tight">{cat.name}</h3>
                    <p className="text-xs text-slate-400 leading-relaxed line-clamp-4">{cat.description}</p>
                  </div>

                  <div className="relative z-10 pt-4 flex items-center justify-between text-[10px] font-bold text-slate-500 group-hover:text-white transition-colors border-t border-slate-900/60 font-mono">
        ...
        {/* INTERACTIVE VIBE TUNING CONTROLLER */}
        <div id="vibe-sandbox" className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch pt-2">
          
          {/* Visual Ambiance Calibrator Card */}
          <div className="lg:col-span-7 rounded-2xl border border-slate-900 bg-slate-950 p-6 sm:p-8 flex flex-col justify-between space-y-6">
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <Sliders className="w-4 h-4 text-amber-500" />
                <h3 className="text-base font-extrabold text-white">Atmospheric Calibration Console</h3>
              </div>
              <p className="text-xs text-slate-400 leading-relaxed">
                AuraLife is coupled with a background stardust engine that adapts to user preference. Use the sliders below to calibrate your visual reading comfort, shift wavelengths, and toggle speed loops.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center text-[10px] font-bold font-mono">
                  <span className="text-slate-400 uppercase">Spectrum (Hue)</span>
                  <span className="text-amber-400">{Math.round(hue)}°</span>
                </div>
                <input 
                  type="range"
                  min="0"
                  ...
                  title="Wavelength Spectrum Slider"
                />
                <span className="text-[9px] text-slate-600 block font-mono">Dynamic stardust wavelength tracking</span>
              </div>

              {/* Particle Density */}
              <div className="space-y-2">
                <div className="flex justify-between items-center text-[10px] font-bold font-mono">
                  <span className="text-slate-400 uppercase">Stardust Density</span>
                  <span className="text-amber-400">{particleCount} Particles</span>
                </div>
                <div className="grid grid-cols-3 gap-1.5 pt-1">
                  {[
                    ...
                    <button
                      key={lvl.value}
                      type="button"
                      onClick={() => setParticleCount(lvl.value)}
                      className={`py-1 text-[9px] font-black rounded border cursor-pointer uppercase font-mono transition-all ${
                        particleCount === lvl.value
                          ? 'bg-amber-500/10 border-amber-500 text-amber-400'
                          : 'bg-slate-900/60 border-slate-850 text-slate-500 hover:text-slate-350'
                      }`}
                    >
                      {lvl.label}
                    </button>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center text-[10px] font-bold font-mono">
                  <span className="text-slate-400 uppercase">Evolution Speed</span>
                  <span className="text-amber-400">{speed === 0 ? 'Paused' : speed < 0.1 ? 'Slow' : 'Standard'}</span>
                </div>
                <div className="grid grid-cols-3 gap-1.5 pt-1">
                  {[
                    { value: 0, label: 'Hold' },
                    { value: 0.05, label: 'Relax' },
                    { value: 0.15, label: 'Flow' }
                  ].map((spd) => (
                    <button
                      key={spd.value}
                      type="button"
                      onClick={() => {
                        ...
                        if (spd.value === 0) {
                          ...
                          setIsHuePaused(false);
                        }
                      }}
                      className={`py-1 text-[9px] font-black rounded border cursor-pointer uppercase font-mono transition-all ${
                        (isHuePaused && spd.value === 0) || (!isHuePaused && speed === spd.value)
                          ? 'bg-amber-500/10 border-amber-500 text-amber-400'
                          : 'bg-slate-900/60 border-slate-850 text-slate-500 hover:text-slate-350'
                      }`}
                    >
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center text-[10px] font-bold font-mono">
                  <span className="text-slate-400 uppercase">Aurora Ambiance</span>
                  <span className="text-amber-400">{Math.round(auroraOpacity * 100)}%</span>
                </div>
                <div className="grid grid-cols-3 gap-1.5 pt-1">
                  {[
                    { value: 0.2, label: 'Muted' },
                    { value: 0.65, label: 'Ambient' },
                    { value: 1.0, label: 'Radiant' }
                  ].map((opac) => (
                    <button
                      key={opac.value}
                      type="button"
                      onClick={() => setAuroraOpacity(opac.value)}
                      className={`py-1 text-[9px] font-black rounded border cursor-pointer uppercase font-mono transition-all ${
                        auroraOpacity === opac.value
                          ? 'bg-amber-500/10 border-amber-500 text-amber-400'
                          : 'bg-slate-900/60 border-slate-850 text-slate-500 hover:text-slate-350'
                      }`}
                    >
            ...
            <div className="p-3.5 bg-slate-900/50 rounded-xl border border-slate-850/60 flex items-center gap-3">
              <div className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-ping shrink-0" />
              <p className="text-[11px] text-slate-400 leading-tight">
                <strong>System Feedback:</strong> Ambient atmosphere connected. Shifting to color coordinates with real-time stardust emission cycles.
              </p>
            </div>
          </div>

          {/* Premium Newsletter Card with interactive simulation */}
          <div className="lg:col-span-5 rounded-2xl border border-slate-900 bg-slate-950 p-6 sm:p-8 flex flex-col justify-between space-y-6">
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <Send className="w-4 h-4 text-amber-500" />
                <h3 className="text-base font-extrabold text-white">Join the Newsletter</h3>
              </div>
              <p className="text-xs text-slate-400 leading-relaxed">
                Subscribe to receive premium weekly updates on cognitive masteries, culinary crafts, architecture highlights, and visual diaries. No fluff, only the finest.
              </p>
            </div>
            ...
            {emailStatus === 'success' ? (
              <motion.div
                ...
                className="p-5 rounded-xl bg-gradient-to-br from-slate-900 to-slate-950 border border-amber-500/20 text-center space-y-3 relative overflow-hidden"
              >
                <div className="absolute top-0 left-0 right-0 h-[2px] bg-amber-500" />
                <div className="w-10 h-10 rounded-full bg-amber-500/10 flex items-center justify-center mx-auto text-amber-400 text-lg font-black">✓</div>
                <h4 className="text-xs font-black uppercase tracking-widest text-[#fbbf24] font-mono font-bold font-sans">Curation Pass Unlocked</h4>
                <div className="border border-slate-800 p-3 rounded bg-black/60 text-left font-mono space-y-1">
                  <div className="text-[9px] text-slate-500 flex justify-between"><span>PASS TYPE:</span> <span className="text-white">PREMIUM READER</span></div>
                  <div className="text-[9px] text-slate-500 flex justify-between"><span>MEMBER ID:</span> <span className="text-amber-400 font-bold">#AL-93282</span></div>
                  <div className="text-[9px] text-slate-500 flex justify-between"><span>EMAIL:</span> <span className="text-slate-300 truncate max-w-[150px]">{email}</span></div>
                </div>
                <p className="text-[10px] text-slate-400 leading-normal">Welcome aboard. The next curation journal arrives shortly.</p>
              </motion.div>
            ) : (
              <form onSubmit={handleNewsletterSubmit} className="space-y-3">
                <div className="space-y-1">
                  <label htmlFor="newsletter-email" className="text-[10px] font-bold text-slate-500 uppercase font-mono block">Email Address</label>
                  <input 
                    type="email"
                    id="newsletter-email"
                    required
                    ...
                    className="w-full bg-slate-900 border border-slate-850 p-3 rounded-lg text-xs font-sans text-white placeholder-slate-600 focus:outline-none focus:border-amber-500 transition-colors"
                  />
                </div>
                <button
                  type="submit"
                  disabled={emailStatus === 'submitting'}
                  className="w-full py-2.5 bg-slate-100 hover:bg-white text-slate-950 font-black rounded-lg text-[10px] uppercase tracking-widest cursor-pointer transition-all flex items-center justify-center gap-2"
                >
                  {emailStatus === 'submitting' ? 'Securing Access...' : 'Secure Curation Pass'}
                </button>
              </form>
            )}

            <p className="text-[10px] text-slate-500 text-center font-mono">
              ★ Fully GDPR-compliant. Zero marketing. Opt-out anytime.
            </p>
          </div>

        </div>

        {/* CURATOR COUNCIL SECTION */}
        <div className="space-y-6 pt-4">
          <div className="text-center space-y-1.5">
            <h2 className="text-xl sm:text-2xl font-extrabold text-white tracking-tight">Meet the Editorial Board</h2>
            <p className="text-xs text-slate-500 font-mono">Deserving perspectives from our curators, consultants, and experts</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {profiles.map((curator, idx) => {
              const isFollowed = followedCurators.includes(curator.id);
              const ratingScore = 9.8 - (idx * 0.2);
              return (
                <div 
                  key={curator.id}
                  className="rounded-2xl border border-slate-900 bg-slate-950 p-6 flex flex-col justify-between space-y-6 relative overflow-hidden"
                >
                  <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl from-slate-900/50 to-transparent rounded-bl-3xl" />
                  
                  <div className="space-y-4">
                    {/* Curator Avatar and info */}
                    <div className="flex items-center gap-3 relative z-10">
                      <img src={curator.avatarUrl} className="w-12 h-12 rounded-full object-cover border border-slate-850" alt="" />
                      <div>
                        <h4 className="text-sm font-black text-white">{curator.fullName}</h4>
                        <span className="text-[9px] text-[#fbbf24] uppercase font-bold font-mono tracking-widest block">
                          {curator.role === 'admin' ? 'Curation Architect' : curator.role === 'editor_in_chief' ? 'Food Editor' : 'Cognitive Specialist'}
                        </span>
                      </div>
                    </div>

                    <p className="text-xs text-slate-400 leading-relaxed font-sans">{curator.bio}</p>
                  </div>

                  <div className="pt-4 border-t border-slate-900/80 flex items-center justify-between text-xs font-bold font-mono">
                    <div className="flex flex-col">
                      <span className="text-[8px] text-slate-600 block">AURA SCORE</span>
                      <span className="text-[#fbbf24]">{ratingScore.toFixed(1)} ★</span>
                    </div>

                    <button
                      onClick={() => {
                        if (isFollowed) {
                          setFollowedCurators(prev => prev.filter(id => id !== curator.id));
                        } else {
                          setFollowedCurators(prev => [...prev, curator.id]);
                        }
                      }}
                      className={`px-3 py-1.5 rounded-lg border text-[10px] uppercase font-black tracking-widest cursor-pointer transition-all flex items-center gap-1 ${
                        isFollowed 
        ...
        {/* ADMIN WORKSPACE FOOTER PORTAL CTA */}
        <div className="rounded-2xl border border-dashed border-slate-850 p-8 flex flex-col sm:flex-row items-center justify-between gap-6 bg-slate-950/20">
          <div className="space-y-1.5 text-center sm:text-left">
            <h4 className="text-sm font-bold text-white">Pristine Production Environment Ready</h4>
            <p className="text-xs text-slate-400 max-w-xl">
              Authentications are set to permit any valid email and password combo. Log in to the workspace to create channels, publish stories, and curate content live.
            </p>
          </div>
          <button
            onClick={onNavigateToAdmin}
            className="px-5 py-2.5 rounded-lg bg-amber-600 hover:bg-amber-700 text-slate-950 font-black text-xs uppercase tracking-widest cursor-pointer transition-all flex items-center gap-1.5 shadow"
          >
      ...
      <nav className="border-b border-slate-900/90 bg-slate-950/80 backdrop-blur-md sticky top-0 z-40">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={onNavigateToAdmin}
              className="px-3 py-1.5 rounded-lg bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-500 hover:to-amber-600 text-white font-black cursor-pointer transition-all flex items-center gap-1.5 text-xs shadow-[0_4px_12px_rgba(217,119,6,0.25)] border border-amber-500/20 animate-fade-in"
              title="Admin Workspace Console"
            >
